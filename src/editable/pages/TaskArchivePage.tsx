    ...
    <EditableSiteShell>
      <main className="bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto grid max-w-[var(--editable-container)] gap-6 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_260px]">
          <div className="border-b border-black/12 bg-[var(--slot4-surface-bg)] pb-8">
            <div className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.16em] text-[var(--slot4-accent)]"><Icon className="h-4 w-4" /> {label}</div>
            ...
            <div className="mt-5 border-l-4 border-[var(--slot4-accent-fill)] bg-[var(--slot4-gray)] p-4 text-sm font-bold leading-7 text-neutral-700">{deck.promise}</div>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={basePath} className="bg-[var(--slot4-dark-bg)] px-5 py-3 text-sm font-black text-[var(--slot4-page-bg)]">Browse all</Link>
              <Link href="/search" className="border border-black/[0.09] px-5 py-3 text-sm font-black">Search articles</Link>
            </div>
          </div>

          <form action={basePath} className="self-end border border-black/[0.09] bg-[var(--slot4-gray)] p-5">
            ...
            <select name="category" defaultValue={category} className="mt-4 h-12 w-full border border-black/[0.09] bg-white px-4 text-sm font-bold outline-none">
              <option value="all">All categories</option>
              {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
            </select>
            <button className="mt-3 h-12 w-full bg-[var(--slot4-dark-bg)] text-sm font-black text-[var(--slot4-page-bg)]">Apply</button>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            ...
            <span className="bg-[var(--slot4-dark-bg)] px-5 py-3 text-sm font-black text-[var(--slot4-page-bg)]">Page {page} of {pagination.totalPages || 1}</span>
  ...
  return (
    <Link href={href} className="group grid gap-4 border-b border-black/10 bg-white py-5 transition hover:bg-neutral-50 sm:grid-cols-[230px_minmax(0,1fr)]">
      ...
      <div className="min-w-0">
        <p className="text-[11px] font-black uppercase tracking-[0.14em] text-[var(--slot4-accent)]">Article {String(index + 1).padStart(2, '0')}</p>
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group grid gap-5 rounded-[2rem] border border-black/[0.09] bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl sm:grid-cols-[120px_1fr]">
      <div className="flex h-28 w-28 items-center justify-center overflow-hidden rounded-[1.5rem] bg-[var(--slot4-page-bg)] ring-1 ring-black/[0.09]">
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap gap-2">
          ...
          {location ? <span className="inline-flex items-center gap-1 rounded-full border border-black/[0.09] px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em]"><MapPin className="h-3 w-3" /> {location}</span> : null}
        </div>
...
function ClassifiedArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group overflow-hidden rounded-[2rem] border border-black/[0.09] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-[2rem] border border-black/[0.09] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
...
function BookmarkArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className="group block rounded-[1.7rem] border border-black/[0.09] bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:bg-[var(--slot4-dark-bg)] hover:text-[var(--slot4-page-bg)]">
...
function PdfArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className="group rounded-[2rem] border border-black/[0.09] bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
...
function ProfileArchiveCard({ post, href, showImage }: { post: SitePost; href: string; showImage: boolean }) {
  ...
  return (
    <Link href={href} className="group rounded-[2rem] border border-black/[0.09] bg-white p-6 text-center shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
