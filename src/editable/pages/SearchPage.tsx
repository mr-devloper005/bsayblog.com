...
const toPlainText = (value: string) => stripHtml(value).replace(/\s+/g, ' ').trim()
...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-black/[0.09] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
      ...
      <div className="p-5 sm:p-6">
        ...
        {summary ? <p className="mt-4 line-clamp-3 text-sm font-semibold leading-7 text-[var(--slot4-page-text)]/65">{toPlainText(summary)}</p> : null}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.5rem] border border-black/[0.09] bg-white/70 p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
            ...
            <form action="/search" className="self-end rounded-[2rem] border border-black/[0.09] bg-[var(--slot4-page-bg)] p-4 sm:p-5">
              <input type="hidden" name="master" value="1" />
              <label className="flex items-center gap-3 rounded-2xl border border-black/[0.09] bg-white px-4 py-3">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label className="flex items-center gap-2 rounded-2xl border border-black/[0.09] bg-white px-4 py-3">
                ...
                <select name="task" defaultValue={task} className="rounded-2xl border border-black/[0.09] bg-white px-4 py-3 text-sm font-black outline-none">
                  <option value="">All content types</option>
                  {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[var(--slot4-page-text)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--slot4-page-bg)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/article" className="inline-flex items-center gap-2 rounded-full border border-black/[0.09] bg-white px-5 py-3 text-sm font-black">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-black/[0.09] bg-white/70 p-10 text-center">
