...
const fieldClass = 'rounded-2xl border border-black/[0.09] bg-white px-4 py-3 text-sm font-bold text-[var(--slot4-page-text)] outline-none transition placeholder:text-current/35 focus:border-current'
    ...
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[var(--slot4-page-bg)] px-4 py-16 text-[var(--slot4-page-text)] sm:px-6 lg:px-8">
          <section className="mx-auto grid max-w-5xl gap-8 rounded-[2.8rem] border border-black/[0.09] bg-white/75 p-7 shadow-[0_30px_90px_rgba(15,23,42,0.08)] md:grid-cols-[0.9fr_1.1fr] md:p-10">
            <div className="flex h-full min-h-72 items-center justify-center rounded-[2rem] bg-[var(--slot4-page-text)] text-[var(--slot4-page-bg)]">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="inline-flex items-center gap-2 rounded-full bg-[var(--slot4-page-text)] px-6 py-3 text-sm font-black text-[var(--slot4-page-bg)]">Login <ArrowRight className="h-4 w-4" /></Link>
                <Link href="/signup" className="inline-flex items-center gap-2 rounded-full border border-black/[0.09] bg-white px-6 py-3 text-sm font-black">Sign up</Link>
              </div>
            </div>
          </section>
        </main>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.8rem] border border-black/[0.09] bg-white/75 p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] backdrop-blur lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-current bg-[var(--slot4-page-text)] text-[var(--slot4-page-bg)]' : 'border-black/[0.09] bg-white hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-[2.2rem] border border-black/[0.09] bg-[var(--slot4-page-bg)] p-5 sm:p-7">
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[var(--slot4-page-text)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--slot4-page-bg)] transition hover:-translate-y-0.5">
