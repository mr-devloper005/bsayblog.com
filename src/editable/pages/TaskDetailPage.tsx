...
import { ArrowLeft, Award, Bookmark, Briefcase, Building2, CalendarDays, Camera, CheckCircle2, ChevronRight, Download, ExternalLink, FileText, Flame, Globe2, Heart, Mail, MapPin, MessageCircle, MessageSquare, Phone, Share2, Tag, UserRound, Users } from 'lucide-react'
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <div className="relative min-h-screen bg-[var(--slot4-page-bg)]">
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <article className="rounded-[2.8rem] border border-black/[0.09] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.09)] sm:p-9">
          <div className="grid gap-6 sm:grid-cols-[150px_1fr]">
            <div className="flex h-36 w-36 items-center justify-center overflow-hidden rounded-[2rem] bg-[var(--slot4-page-bg)] ring-1 ring-black/[0.09]">
...
function ClassifiedDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-16">
      <aside className="rounded-[2.5rem] border border-black/[0.09] bg-[var(--slot4-dark-bg)] p-7 text-[var(--slot4-page-bg)] shadow-xl lg:sticky lg:top-24 lg:self-start">
      ...
      <article className="rounded-[2.7rem] border border-black/[0.09] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-9">
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="rounded-[2.5rem] border border-black/[0.09] bg-white p-7 lg:sticky lg:top-24 lg:self-start">
        ...
        <div className="columns-1 gap-5 space-y-5 md:columns-2">
          {(images.length ? images : ['/placeholder.svg?height=900&width=1200']).map((image, index) => (
            <figure key={`${image}-${index}`} className="break-inside-avoid overflow-hidden rounded-[2rem] border border-black/[0.09] bg-white shadow-sm">
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.7rem] border border-black/[0.09] bg-white p-7 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-10">
        <BackLink task="sbm" />
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.7rem] border border-black/[0.09] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] sm:p-9">
        ...
        <BodyContent post={post} />
        {fileUrl ? (
          <div className="mt-8 overflow-hidden rounded-[2rem] border border-black/[0.09] bg-[var(--slot4-page-bg)]">
...
function ArticleDetailActions(_props: { slug: string; comments: number }) {
  return (
    <div className="flex items-center gap-3">
  ...
  return (
    <div className="mt-8 grid gap-3 sm:grid-cols-2">
      {visible.map(([label, value, Icon]) => (
        <div key={label} className="rounded-[1.5rem] border border-black/[0.09] bg-[var(--slot4-page-bg)] p-4">
  ...
  return (
    <section className="mt-8">
      ...
      <div className={`mt-4 grid gap-3 ${large ? 'sm:grid-cols-2' : 'grid-cols-2 sm:grid-cols-4'}`}>
        {images.slice(0, large ? 4 : 8).map((image, index) => <img key={`${image}-${index}`} src={image} alt="" className="aspect-[4/3] rounded-[1.4rem] object-cover ring-1 ring-black/[0.09]" />)}
      </div>
    </section>
  )
}
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="overflow-hidden rounded-[2rem] border border-black/[0.09] bg-white shadow-sm">
...
function ContactAction({ website, phone, email }: { website?: string; phone?: string; email?: string }) {
  ...
  return (
    <div className="mt-5 rounded-[2rem] border border-black/[0.09] bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        {phone ? <a href={`tel:${phone}`} className="inline-flex items-center gap-2 rounded-full border border-black/[0.09] px-4 py-2 text-sm font-black"><Phone className="h-4 w-4" /> Call</a> : null}
        {email ? <a href={`mailto:${email}`} className="inline-flex items-center gap-2 rounded-full border border-black/[0.09] px-4 py-2 text-sm font-black"><Mail className="h-4 w-4" /> Email</a> : null}
      </div>
    </div>
  )
}
...
function RelatedPanel({ task, post: _post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      ...
      {related.length ? (
        <div className="border border-black/[0.09] bg-white p-5">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-b border-black/10 bg-white py-3 transition hover:text-[var(--slot4-accent-fill)]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-16 w-16 shrink-0 object-cover" /> : <div className="flex h-16 w-16 shrink-0 items-center justify-center bg-[var(--slot4-page-bg)]"><FileText className="h-6 w-6 opacity-45" /></div>}
      <div className="min-w-0">
    ...
    <section className="mt-10 border-t border-black/12 pt-7">
      ...
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="border border-black/[0.09] bg-white p-4">
